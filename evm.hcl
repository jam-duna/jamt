job "jam-evm" {
  datacenters = ["dc1"]
  type = "service"

  group "evm" {
    count = 1

    constraint {
      attribute = "${node.unique.name}"
      value     = "coretime.us-central1-c.c.jamduna.internal"
    }

    restart {
      attempts = 10
      interval = "5m"
      delay    = "15s"
      mode     = "delay"
    }

    task "evm-stream" {
      driver = "raw_exec"

      config {
        command = "/bin/bash"
        args = [
          "-c",
          <<EOF
set -e
export HOME=/root
echo "Running EVM stream runner on coretime"

# Must point to the directory where the JAM release has been extracted.

echo "Cleaning previous fib builder DB: /root/.jamduna/jam-6/leveldb"
rm -rf /root/.jamduna/jam-6/leveldb

BASE_DIR="${NOMAD_TASK_DIR}"
mkdir -p "$BASE_DIR/local"
mkdir -p /root/.jamduna/keys

curl -fsSL -o "$BASE_DIR/local/evm-stream-runner" http://coretime.jamduna.org/jamduna/evm-stream-runner
chmod +x "$BASE_DIR/local/evm-stream-runner"

curl -fsSL -o "$BASE_DIR/local/evm-builder" http://coretime.jamduna.org/jamduna/evm-builder
chmod +x "$BASE_DIR/local/evm-builder"

curl -fsSL -o "$BASE_DIR/local/evm-feeder" http://coretime.jamduna.org/jamduna/evm-feeder
chmod +x "$BASE_DIR/local/evm-feeder"

curl -fsSL -o "$BASE_DIR/local/spec.json" http://coretime.jamduna.org/jamduna/spec.json

for i in 1 2 3 4 5 6; do
  curl -fsSL -o /root/.jamduna/keys/seed_$i \
  http://coretime.jamduna.org/jamduna/keys/seed_$i
done

# Download jamduna binary and display Peer ID + Bandersnatch key for each validator
curl -fsSL -o "$BASE_DIR/local/jamduna" http://coretime.jamduna.org/jamduna/jamduna
chmod +x "$BASE_DIR/local/jamduna"
echo "=== Validator Key Info ==="
"$BASE_DIR/local/jamduna" list-keys -d /root/.jamduna

export EVM_RUNNER_BUILDER_BIN="$BASE_DIR/local/evm-builder"
export EVM_RUNNER_FEEDER_BIN="$BASE_DIR/local/evm-feeder"

exec "$BASE_DIR/local/evm-stream-runner" \
  -chain "$BASE_DIR/local/spec.json"
EOF
        ]
      }

      resources {
        cpu    = 2000
        memory = 4096
      }
    }
  }
}
